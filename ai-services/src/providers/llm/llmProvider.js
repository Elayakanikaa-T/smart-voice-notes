...
class MockLLMProvider {
    async generateSummary(transcript, title) {
        logger_js_1.logger.info(`[LLM:Mock] Generating AI summary for lecture note...`);
        await new Promise(resolve => setTimeout(resolve, 600));
        return {
            summaryText: 'This lecture examined shortest path graph algorithms, contrasting Dijkstra’s greedy approach with A* heuristic search. Key constraints such as non-negative edge weights and priority queue data structures were evaluated alongside algorithmic complexities. An upcoming midterm exam date was also announced.',
            bulletPoints: [
    ...
    async generateFlashcards(transcript, count = 5) {
        logger_js_1.logger.info(`[LLM:Mock] Generating ${count} study flashcards...`);
    ...
    async generateQuiz(transcript, count = 5, difficulty = 'medium') {
        logger_js_1.logger.info(`[LLM:Mock] Generating ${count} MCQ quiz questions at ${difficulty} level...`);
        await new Promise(resolve => setTimeout(resolve, 600));
        return [
            {
                questionId: 'q-1',
