import { pgPool, isPostgresConnected, memoryStore } from '../../../backend/src/config/database';
...
export async function calculateSubjectReadiness(
  userId: string,
  subjectId: string
): Promise<ReadinessCalculationResult> {
  let attempts: any[] = [];
  let noteCount = 0;
  ...
  if (isPostgresConnected && pgPool) {
    ...
    const notesRes = await pgPool.query(
      `SELECT COUNT(*)::int as total_notes,
       ...
       FROM notes_meta
       WHERE user_id = $1 AND subject_id = $2 AND is_archived = false`,
      [userId, subjectId]
    );
    ...
    for (const a of memoryStore.quizAttempts.values()) {
      if (a.userId === userId) {
        const q = memoryStore.quizzes.get(a.quizId);
        if (q && q.subject_id === subjectId) {
          attempts.push(a);
        }
      }
    }
    for (const n of memoryStore.notesMeta.values()) {
      if (n.userId === userId && n.subjectId === subjectId && !n.isArchived) {
        noteCount++;
        if (n.status === 'ready') reviewedNoteCount++;
      }
    }
  }
  ...
  const incorrectTopicsCount = new Map<string, number>();
  ...
  if (attempts.length > 0) {
    ...
    if (diffDays <= 1) reviewRecencyScore = 100;
    else if (diffDays <= 3) reviewRecencyScore = 85;
    else if (diffDays <= 7) reviewRecencyScore = 65;
    else reviewRecencyScore = Math.max(20, 100 - Math.round(diffDays * 8));
  }
...
export async function updateSubjectReadinessScore(userId: string, subjectId: string): Promise<void> {
  try {
    const result = await calculateSubjectReadiness(userId, subjectId);

    if (isPostgresConnected && pgPool) {
      await pgPool.query(
        `INSERT INTO progress_scores (user_id, subject_id, readiness_score, quiz_accuracy_avg, material_coverage_pct, review_recency_score, weak_areas_json, updated_at)
      ...
      memoryStore.progressScores.set(`${userId}_${subjectId}`, {
        user_id: userId,
    ...
    logger.info(`[Worker:Readiness] Updated readiness score=${result.readinessScore}% for subject=${subjectId}`);
  } catch (error) {
    logger.error(`[Worker:Readiness] Failed to update readiness score:`, error);
  }
}
